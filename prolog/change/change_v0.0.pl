fewest_coins(Coins, Target, Change) :-
    ...
    aggregate_all(min(N), (member(List, PossibleChanges), count_coins(List, N)), MinimumCoins),
    member(ReversedChange, PossibleChanges),
    count_coins(ReversedChange, MinimumCoins),
...
possible_changes(Coins, Target, Change) :-
    sort(Coins, SortedCoins),
    reverse(SortedCoins, ReversedSortedCoins),
