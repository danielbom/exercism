...
parse_rest(Stream0, [Op, Number | MathTokens], Rest) :-
  parse_assert(parse_operation(Stream0, Op, Stream1), unknown_operation_error),
  parse_string(Stream1, " ", Stream2),
  parse_assert(parse_integer(Stream2, Number, Stream3), syntax_error),
  parse_rest(Stream3, MathTokens, Rest).
